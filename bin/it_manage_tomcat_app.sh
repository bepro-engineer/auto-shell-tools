...
# 目的：
#   -f（ファイル指定）で「アプリ操作対象一覧ファイル」を渡したとき、
#   ファイル内がすべてコメント行の場合でも正常終了（rc=0）することを確認する。
# 想定ファイル：
#   運用者が作成するアプリ一覧ファイルに、誤ってコメント行しか記載されていないケース。
# ファイル内容：
#   コメント行のみ（全行 # で始まる）
# ------------------------------------------------------------
...
# 目的：
#   -f（ファイル指定）で「アプリ操作対象一覧ファイル」を渡したとき、
#   同一アプリパスが重複していても処理が落ちず、
#   WARNING（rc=1）として集計され最終 rc=1 になることを確認する。
# 想定ファイル：
#   運用者が作成するアプリ一覧ファイルに、
#   手作業やコピペミスにより同一アプリパスが重複して記載されているケース。
# ファイル内容：
#   コメント行（無視）
#   同一アプリパス
#   同一アプリパス
...
# 目的：
#   -f（ファイル指定）で start を実行した際、
#   実在アプリと存在しないアプリが混在している場合に、
#   ERROR（rc=2）として集計され最終 rc=2 になることを確認する。
# 想定ファイル：
#   運用者が作成するアプリ一覧ファイルに、
#   存在しないアプリパスが誤って混入しているケース。
# ファイル内容：
#   コメント行（無視）
#   実在するアプリパス
#   存在しないアプリパス
...
# 目的：
#   -f（ファイル指定）で stop を実行した際、
#   同一アプリパスが重複していても処理が落ちず、
#   WARNING（rc=1）として集計され最終 rc=1 になることを確認する。
# 想定ファイル：
#   運用者が作成するアプリ一覧ファイルに、
#   停止対象アプリのパスが重複して記載されているケース。
# ファイル内容：
#   コメント行（無視）
#   同一アプリパス
#   同一アプリパス
...
# 目的：
#   -f（ファイル指定）で stop を実行した際、
#   実在アプリと存在しないアプリが混在している場合でも、
#   stop の仕様どおり WARNING（rc=1）として集計されることを確認する。
# 想定ファイル：
#   運用者が作成するアプリ一覧ファイルに、
#   既に削除済み、または存在しないアプリパスが混在しているケース。
# ファイル内容：
#   コメント行（無視）
#   実在するアプリパス
#   存在しないアプリパス
...
# 目的：
#   -f（ファイル指定）で start を実行した際、
#   アプリパスの前後に空白が含まれていても正しく処理され、
#   WARNING（rc=1）として集計されることを確認する。
# 想定ファイル：
#   運用者が手編集したアプリ一覧ファイルに、
#   行頭・行末の不要な空白が混入しているケース。
# ファイル内容：
#   コメント行（無視）
#   前後に空白を含むアプリパス
#   空白を含まない同一アプリパス
